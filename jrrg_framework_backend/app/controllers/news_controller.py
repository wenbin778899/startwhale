...
def get_market_news():
    ...
    try:
        # 检查缓存是否有效
        if news_cache['market_news']['timestamp'] is not None and \
           (datetime.now() - news_cache['market_news']['timestamp']).total_seconds() < CACHE_EXPIRY:
        ...
        try:
            # 尝试获取东方财富网财经新闻
            response = requests.get('https://finance.eastmoney.com/a/cywjdd.html', headers=headers, timeout=5)
            ...
            if response.status_code == 200:
                ...
                for i, news in enumerate(news_list):
                    ...
                    if news_link:
                        ...
                        # 获取发布时间和来源（如果有）
                        news_info = news.find_next_sibling('p', class_='info')
                        source = '东方财富网'
                        publish_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                        
                        if news_info:
                            info_text = news_info.get_text().strip()
                            if '来源：' in info_text:
                                source = info_text.split('来源：')[1].split()[0]
                            if '发布时间：' in info_text:
                                publish_time = info_text.split('发布时间：')[1].split()[0]
                ...
                # 如果请求失败，使用备用数据
                current_app.logger.warning(f'获取东方财富网新闻失败，状态码: {response.status_code}')
                raise Exception(f'获取东方财富网新闻失败，状态码: {response.status_code}')
                
        except Exception as fetch_error:
            current_app.logger.error(f'抓取东方财富网财经新闻失败: {str(fetch_error)}')
            
            # 如果抓取失败，使用新浪财经作为备用
            try:
                response = requests.get('https://finance.sina.com.cn/roll/index.d.html?cid=56592', headers=headers, timeout=5)
                response.encoding = 'utf-8'
                
                if response.status_code == 200:
                    soup = BeautifulSoup(response.text, 'html.parser')
                    news_list = soup.select('.list_009 li')
                    ...
                    # 如果两个来源都失败，生成模拟数据
                    current_app.logger.warning(f'获取新浪财经新闻失败，状态码: {response.status_code}')
                    market_news = generate_fallback_news(limit)
            except Exception as backup_error:
                current_app.logger.error(f'抓取新浪财经新闻失败: {str(backup_error)}')
                market_news = generate_fallback_news(limit)
        ...
        return utils.error(message=f"获取市场新闻失败: {str(e)}", code=500)
...
def get_topic_news():
    ...
    try:
        ...
        if cache_key in news_cache and news_cache[cache_key]['timestamp'] is not None and \
           (datetime.now() - news_cache[cache_key]['timestamp']).total_seconds() < CACHE_EXPIRY:
        ...
        # 构建搜索URL
        search_url = f'https://search.sina.com.cn/news/search?q={topic}&c=news&sort=time'
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 如果请求失败，使用备用数据
                current_app.logger.warning(f'获取主题新闻失败，状态码: {response.status_code}')
...
def generate_fallback_news(limit):
    ...
    fallback_news = [
        ...
        },
        {
            ...
            'source': '金融时报',
            'publish_time': (datetime.now() - timedelta(hours=10)).strftime('%Y-%m-%d %H:%M:%S')
        },
        {
        ...
        },
        {
            ...
            'source': '上交所',
            'publish_time': (datetime.now() - timedelta(hours=24)).strftime('%Y-%m-%d %H:%M:%S')
        },
        {
            'title': '中央经济工作会议：明年继续实施积极的财政政策和稳健的货币政策',
            'link': 'https://example.com/news/10',
            'source': '中国政府网',
            'publish_time': (datetime.now() - timedelta(hours=36)).strftime('%Y-%m-%d %H:%M:%S')
        }
    ]
        ...
        '科技': [
            {
                ...
                'source': '科技日报',
                'publish_time': (datetime.now() - timedelta(hours=5)).strftime('%Y-%m-%d')
            },
            {
                ...
                'source': '创业邦',
                'publish_time': (datetime.now() - timedelta(hours=8)).strftime('%Y-%m-%d')
            }
        ],
        '金融': [
            {
                ...
                'source': '金融时报',
                'publish_time': (datetime.now() - timedelta(hours=3)).strftime('%Y-%m-%d')
            },
            {
                ...
                'source': '央行网站',
                'publish_time': (datetime.now() - timedelta(hours=7)).strftime('%Y-%m-%d')
            }
        ],
        '医药': [
            {
                ...
                'source': '医药经济报',
                'publish_time': (datetime.now() - timedelta(hours=4)).strftime('%Y-%m-%d')
            },
            {
                ...
                'source': '健康时报',
                'publish_time': (datetime.now() - timedelta(hours=9)).strftime('%Y-%m-%d')
            }
        ]
    }
        ...
        return [
            {
                ...
                'source': '经济参考报',
                'publish_time': (datetime.now() - timedelta(hours=random.randint(1, 12))).strftime('%Y-%m-%d')
            },
            {
